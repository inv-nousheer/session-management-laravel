...
/** Tags as stored CSV or array from API */
...
const allSessionTags = computed(() => {
  ...
  const list = []
  for (const s of sessions.value) {
    for (const tag of normalizeSessionTags(s)) {
...
const filteredSessions = computed(() => {
  ...
  if (filterMemberIds.value.length) {
    ...
    list = list.filter((s) => {
      const ids = memberRows(s).map((m) => m?.users_id).filter(Boolean)
  ...
  if (filterTagsSelected.value.length) {
    ...
    list = list.filter((s) => {
      const tags = normalizeSessionTags(s).map((t) => t.toLowerCase())
      return wantLower.some((w) => tags.includes(w))
    })
  }
...
const duplicateSession = async (sessionId) => {
  ...
  try {
    const created_by = localStorage.getItem('user') ? JSON.parse(localStorage.getItem('user')).id : null
    const res = await api.post(`/api/sessions/${sessionId}/duplicate`, { created_by })
    await fetchSessions()
    const newId = res?.data?.session?.id
    if (newId) {
      // Take user straight to the new copy
      await nextTick()
...
const initializeDatePicker = () => {
  ...
  picker = flatpickr(dateTimeInput.value, {
    ...
    allowInput: false, // ✅ prevents manual/browser interference
    disableMobile: true, // ✅ VERY IMPORTANT (fixes your issue)
...
const submitForm = async () => {
  ...
  try {
    ...
    formError.value = 'Failed to create session. Please try again.'
    console.error('Error creating session:', err)
  } finally {
    submitting.value = false
  }
...
watch(showModal, async (val) => {
  if (val) {
    ...
    // destroy picker when modal closes (important)
    if (picker) {
      picker.destroy()
      picker = null
    }
...
<main class="h-full overflow-y-auto">
    <div class="container px-6 mx-auto grid">
        ...
        <div v-else class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            <div v-for="session in filteredSessions" :key="session.id" class="group relative bg-white dark:bg-gray-800 rounded-xl shadow-md hover:shadow-xl transition-all duration-300 overflow-hidden border border-gray-100 dark:border-gray-700 hover:border-purple-300 dark:hover:border-purple-600">
                ...
                <div class="p-6">
                    ...
                    <!-- Tags -->
                    <div v-if="normalizeSessionTags(session).length" class="flex flex-wrap gap-1.5 mb-4">
                        <span
                            v-for="t in normalizeSessionTags(session)"
                            :key="t"
  ...
  <div v-if="showModal" class="fixed inset-0 z-40" aria-labelledby="modal-title" role="dialog" aria-modal="true">
    <!-- Background overlay -->
    <div class="fixed inset-0 bg-black/50 backdrop-blur-sm transition-opacity z-40" @click="closeModal"></div>
    ...
    <div class="bg-white dark:bg-slate-800 rounded-2xl text-left shadow-2xl transform transition-all w-full max-w-2xl border border-gray-100 dark:border-slate-700 max-h-[90vh] flex flex-col">
        ...
        <div class="px-6 sm:px-8 py-6 space-y-6 overflow-y-auto">
          ...
          <!-- Error Message -->
          <div v-if="formError" class="rounded-lg bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800 p-4">
